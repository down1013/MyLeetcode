...
void combin(vector<vector<int>>& res,vector<int>now,vector<int> candidates,int target)
{
    while(candidates.size())
    {
        ...
        else {now1.push_back(candidates[0]);int a(candidates[0]);candidates.erase(candidates.begin());combin(res,now1,candidates,target-a);
        while(candidates.size()&&a==candidates[0]) candidates.erase(candidates.begin());
        }
    }
